...
async function getFollowedChannels() {
  ...
  try {
    ...
    if (allChannels.length > 0) {
      ...
      displayError("Aucune chaîne suivie trouvée.", "channels-list");
      currentFollowedChannels = [];
      document.getElementById("channels-list").innerHTML = "";
    }
  } catch (error) {
    console.error("Erreur lors de la récupération des chaînes suivies :", error);
    displayError("Erreur lors du chargement des chaînes suivies. Veuillez vous reconnecter.", "channels-list");
    loginButton.style.display = "block";
    currentTwitchToken = null;
    updateLogoutButtonVisibility();
  }
...
async function getYoutubeChannels() {
  ...
  try {
    ...
    displayError("Erreur lors du chargement des chaînes YouTube. Veuillez vous reconnecter.", "channels-list");
    loginButton.style.display = "block";
